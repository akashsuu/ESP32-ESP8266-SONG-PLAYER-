...
extern HardwareSPI SPI;
    ...
    // Progmem is Arduino-specific
        ...
        #include <pgmspace.h>
        #define PRIPSTR "%s"
        /* ESP8266 core 2.5.0 defines pgm_read_ptr() as a void-pointer
         * dereference, which is invalid C++. RF24 diagnostics read pointer
         * tables from PROGMEM, so override only that broken core macro. */
        #if defined(ARDUINO_ARCH_ESP8266)
            #undef pgm_read_ptr
            #define pgm_read_ptr(p) (*(void* const*)(p))
        #elif !defined(pgm_read_ptr)
            #define pgm_read_ptr(p) (*(void* const*)(p))
        #endif
